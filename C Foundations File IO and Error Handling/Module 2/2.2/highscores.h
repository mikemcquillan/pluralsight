...
#define FILE_NAME_HIGH_SCORES   "files\\highscores.txt"
...
void saveHighScores()
{
    FILE *pfile = fopen(FILE_NAME_HIGH_SCORES, "w");
...
void loadHighScores()
{
    FILE *pfile = fopen(FILE_NAME_HIGH_SCORES, "r");
    char tempName[20];
    int tempScore, counter = 0;
    currentHighScores = initializeHighScores();

    if(pfile != NULL)
    {
        while(fscanf(pfile, "%s\t%d", tempName, &tempScore) == 2)
        {
            HighScore hs;
            strcpy(hs.name, tempName);
            hs.score = tempScore;
